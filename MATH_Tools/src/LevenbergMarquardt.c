/*----------------------------------------------------------------————————————————————————--
文件功能：
Levenberg-Marquardt算法，求解非线性方程最优解，结合了高斯牛顿法和梯度下降 
本算法的详细讲解：
https://blog.csdn.net/loveuav/article/details/81592870 移植于BlueSkyFlightControl
----------------------------------------------------------------————————————————————————--*/
...
void LevenbergMarquardt(Vector3f_t inputData[6], Vector3f_t* offset, Vector3f_t* scale, float initBeta[6], float length)
{
    ...
    while(change > eps)
    {
        ...
        /*根据步长大小更新LM因子 LM因子为0时，算法退化为高斯牛顿法，LM因子变大时，退化为步长较小的梯度下降法*/
